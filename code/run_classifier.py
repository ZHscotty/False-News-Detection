...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    with tf.variable_scope('att_fw'):
        att_output = modeling.attention_layer(from_tensor=output_layer, to_tensor=output_layer)
    with tf.variable_scope('att_bw'):
        att_output_reverse = modeling.attention_layer(from_tensor=output_layer_reverse, to_tensor=output_layer_reverse)
    att_output_mix = tf.concat([att_output, att_output_reverse], axis=2)
    with tf.variable_scope('att_final'):
        att_output_final = modeling.attention_layer(from_tensor=att_output_mix, to_tensor=att_output_mix)
    att_output_final = modeling.layer_norm_and_dropout(att_output_final, bert_config.hidden_dropout_prob)
