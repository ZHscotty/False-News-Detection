...
class Model:
    ...
    def __init__(self):
        ...
        self.ll = 3
        self.MODEL_PATH = '../model/cnn_lstm/cnn_lstm_model'
        self.MODEL_DIC = '../model/cnn_lstm/'
        self.x = tf.placeholder(tf.int32, [None, None])
        self.y = tf.placeholder(tf.float32, [None, None])
        self.ACC_PATH = '../result/cnn_lstm_acc'
        self.LOSS_PATH = '../result/cnn_lstm_loss'
    ...
    def run(self):
        ...
        with tf.variable_scope('conv'):
            ...
            conv = tf.nn.conv2d(embedd, w, strides=[1, 1, 1, 1], padding='VALID')
            conv = tf.nn.bias_add(conv, b)
            conv_a = tf.nn.relu(conv)
            conv_p = tf.nn.avg_pool(conv_a, ksize=[1, self.FILTER_SIZE, 1, 1], strides=[1, 1, 1, 1], padding='SAME')
            # shape (batch, maxlen-filter_size+1, 1, filter_num) => (batch, maxlen-filter_size+1, filter_num)
            conv_concat = tf.reshape(conv_p, shape=[-1, self.MAXLEN-self.FILTER_SIZE+1, self.FILTER_NUM])

        # 2.bilstm
        ##定义两个lstm
        cell_fw = tf.contrib.rnn.BasicLSTMCell(self.LSTM_HIDDEN_SIZE)
        cell_bw = tf.contrib.rnn.BasicLSTMCell(self.LSTM_HIDDEN_SIZE)
        lstm_out, _ = tf.nn.bidirectional_dynamic_rnn(cell_fw=cell_fw, cell_bw=cell_bw, inputs=conv_concat,
                                                      dtype=tf.float32, )
        # 取最后一个cell状态作为句子的表示
