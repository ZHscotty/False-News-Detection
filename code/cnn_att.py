...
class Model:
    ...
    def __init__(self):
        ...
        self.ll = 3
        self.MODEL_PATH = '../model/cnn_att/cnn_att_model'
        self.MODEL_DIC = '../model/cnn_att/'
        self.x = tf.placeholder(tf.int32, [None, None])
        self.y = tf.placeholder(tf.float32, [None, None])
        self.ACC_PATH = '../result/cnn_att_acc'
        self.LOSS_PATH = '../result/cnn_att_loss'
        self.mode = None
    ...
    def run(self):
        ...
        # 2.cnn
        filter_shape = [self.FILTER_SIZE, self.EMBED_SIZE, self.FILTER_NUM]
        with tf.variable_scope('cnn'):
            ...
            cnn = tf.nn.conv1d(embedd, w, stride=1, padding='SAME')
            # shape = (batch, maxlen-k+1, 1, filter_num)
            cnn = tf.nn.bias_add(cnn, b)
        # shape = (batch, maxlen-k+1, filter_num)
        cnn_output = tf.reshape(cnn, shape=[-1, self.MAXLEN, self.FILTER_NUM])

        att_output = self.attention('cnn_att', cnn_output, self.FILTER_NUM, self.ATTENTION_SIZE)
